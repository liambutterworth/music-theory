...
use App\Domain\Theory\Support\Exceptions\IntervalNotFoundException;
...
class Interval
{
    ...
    {
        $attributes = static::all()->firstWhere('name', $name);

        if (is_null($attributes)) {
            throw new IntervalNotFoundException("Interval with name $name not found");
        }

        return new static($attributes);
    }

    public function byAbbr(string $abbr)
    {
        $attributes = static::all()->firstWhere('abbr', $abbr);

        if (is_null($attributes)) {
            throw new IntervalNotFoundException("Interval with abbr $abbr not found");
        }

        return new static($attributes);
    }

    public static function fromFormula(string $formula): Collection
    {
        return static::all()
